...
def file_output(path,scenario,k,tr,inflow_scenario,inflow,need,properties,autoreg):
#	path = 'out.txt'

	output_file = open(path,'w')
#Model Information
	if inflow_scenario=='arima':
		name_of_in_scenario=str(inflow_scenario)+"_"+str(autoreg)
	else:
		name_of_in_scenario=str(inflow_scenario)
	output_file.write("Scenario="+str(scenario)+" Reserves="+str(k)+\
	" Tax_Rate="+str(tr)+" Inflow="+name_of_in_scenario)
